...
- [ ] Enable S-C only if a zero-API candidate passes all four admission controls and fills its pool but fails the inner downstream gate; this condition is now met. Before any request, write an API budget plan, confirm `DASHSCOPE_API_KEY` configuration without exposing a secret, and obtain/record explicit user authorization if credentials or endpoint configuration are unresolved.
...
- [ ] Run the amendment-3 base-only capacity extension and write its balanced-pool decision. Do not run S-C downstream unless it reaches exactly 20 admitted samples in every class.
